...
class TestValidation(unittest.TestCase):
    ...
    def test_valid_ucastIP(self, mock_local_addr):
        bootstrap._context = mock.Mock(local_ip_list=["10.10.10.2", "10.10.10.3"])
        bootstrap.Validation.valid_ucastIP("10.10.10.1")
        mock_local_addr.assert_called_once_with(["10.10.10.2", "10.10.10.3"])
    
    @mock.patch('crmsh.bootstrap.Validation._is_local_addr')
    def test_valid_mcastIP(self, mock_local_addr):
        bootstrap._context = mock.Mock(local_ip_list=["10.10.10.2", "10.10.10.3"],
                local_network_list=["10.10.10.0"])
        bootstrap.Validation.valid_mcastIP("10.10.10.1")
    ...
    def test_valid_adminIP_in_use(self, mock_ipv6, mock_invoke):
        mock_ipv6.return_value = False
        mock_invoke.return_value = True

        with self.assertRaises(ValueError) as err:
            self.validate_inst.valid_adminIP("10.10.10.1")
    ...
    @mock.patch('crmsh.bootstrap.invoke')
    @mock.patch('crmsh.utils.IP.is_ipv6')
    def test_valid_adminIP_not_in_network(self, mock_ipv6, mock_invoke, mock_interfaces):
        mock_ipv6.return_value = False
        mock_invoke.return_value = False
        mock_interfaces_inst = mock.Mock()
        mock_interfaces.return_value = mock_interfaces_inst
        mock_interfaces_inst.ip_in_network.return_value = False
        bootstrap._context.interfaces_inst = mock_interfaces_inst
        ...
        mock_ipv6.assert_called_once_with("10.10.10.1")
        mock_invoke.assert_called_once_with("ping -c 1 10.10.10.1")
        mock_interfaces_inst.ip_in_network.assert_called_once_with("10.10.10.1")
