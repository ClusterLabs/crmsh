...
class TestWatchdog(unittest.TestCase):
    ...
    def test_configure_and_load_driver_cluster_wide(self, mock_run):
        # node_list=None lets cluster_run_cmd discover the node list itself
        # from the running CIB.
        watchdog.Watchdog._configure_and_load_driver("softdog", node_list=None)
        mock_run.assert_called_once_with(
            f"echo softdog > {watchdog.Watchdog.WATCHDOG_CFG} && systemctl restart systemd-modules-load", None)

    @mock.patch('crmsh.utils.cluster_run_cmd')
    def test_configure_and_load_driver_specific_nodes(self, mock_run):
        watchdog.Watchdog._configure_and_load_driver("softdog", node_list=["node1"])
        mock_run.assert_called_once_with(
            f"echo softdog > {watchdog.Watchdog.WATCHDOG_CFG} && systemctl restart systemd-modules-load", ["node1"])
...
Driver: iTCO_wdt
    ...
    @mock.patch('crmsh.watchdog.Watchdog._set_input')
    @mock.patch('crmsh.watchdog.Watchdog._set_watchdog_info')
    def test_init_watchdog_cluster_running(self, mock_set_info, mock_set_input, mock_valid, mock_run, mock_is_loaded, mock_load, mock_get_device):
    ...
    @mock.patch('crmsh.watchdog.Watchdog._set_watchdog_info')
    @mock.patch('crmsh.utils.this_node')
    def test_init_watchdog_cluster_not_running(self, mock_this_node, mock_set_info, mock_set_input, mock_valid, mock_run, mock_is_loaded, mock_load, mock_get_device):
