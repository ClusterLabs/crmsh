...
class TestSBDTimeoutChecker(unittest.TestCase):
    ...
    def test_check_and_fix_not_fix(self, mock_service_manager):
        ...
        self.instance_check._load_configurations = Mock()
        ...
        mock_service_manager_inst.service_is_active.assert_called_once_with(constants.SBD_SERVICE)
        self.instance_check._check_config_consistency.assert_called_once()
        self.instance_check._load_configurations.assert_called_once()
    ...
    def test_check_and_fix_fix_failure(self, mock_service_manager):
        ...
        self.instance_fix._load_configurations = Mock()
    ...
    def test_check_and_fix_fix_success(self, mock_service_manager):
        ...
        self.instance_fix._check_config_consistency = Mock(return_value=True)
        self.instance_fix._load_configurations = Mock()
        self.instance_fix._check_sbd_disk_metadata = Mock(return_value=True)
        self.instance_fix._check_sbd_watchdog_timeout = Mock(return_value=True)
        self.instance_fix._check_sbd_delay_start = Mock(return_value=True)
        self.instance_fix._check_sbd_systemd_start_timeout = Mock(return_value=True)
        self.instance_fix._check_stonith_watchdog_timeout = Mock(return_value=True)
        self.instance_fix._check_stonith_timeout = Mock(return_value=True)

        self.assertTrue(self.instance_fix.check_and_fix())

        mock_service_manager_inst.service_is_active.assert_called_once_with(constants.SBD_SERVICE)
        self.instance_fix._check_config_consistency.assert_called_once()
        self.instance_fix._load_configurations.assert_called_once()
    ...
    def test_check_config_consistency(self, mock_this_node, mock_list_cluster_nodes_except_me, mock_corosync_conf, mock_remote_diff_this, mock_logger_warning, mock_get_sbd_device_from_config, mock_check_devices_metadata_consistent):
    ...
    @patch('logging.Logger.warning')
    def test_check_sbd_disk_metadata_failure(self, mock_logger_warning):
        self.instance_check.disk_based = True
        self.instance_check.sbd_msgwait = 1
        self.instance_check.get_sbd_msgwait_expected = Mock(return_value=10)
        self.assertFalse(self.instance_check._check_sbd_disk_metadata())
        mock_logger_warning.assert_called_once_with("It's recommended that SBD msgwait(now %d) >= %d", 1, 10)

    def test_check_sbd_disk_metadata_success(self):
        self.instance_check.disk_based = True
        self.instance_check.sbd_msgwait = 15
        self.instance_check.get_sbd_msgwait_expected = Mock(return_value=10)
        self.assertTrue(self.instance_check._check_sbd_disk_metadata())

    @patch('crmsh.sh.cluster_shell')
    @patch('logging.Logger.info')
    def test_fix_sbd_disk_metadata(self, mock_logger_info, mock_cluster_shell):
        self.instance_fix.sbd_watchdog_timeout = 5
        ...
        mock_logger_info.assert_called_once_with("Adjusting sbd msgwait to %d", 10)

    @patch('logging.Logger.warning')
    def test_check_sbd_watchdog_timeout_failure(self, mock_logger_warning):
        self.instance_check.disk_based = False
        self.instance_check.sbd_watchdog_timeout = 3
        self.instance_check.qdevice_sync_timeout = 10
        self.assertFalse(self.instance_check._check_sbd_watchdog_timeout())
        mock_logger_warning.assert_called_once_with("It's recommended that SBD_WATCHDOG_TIMEOUT(now %d) >= qdevice sync timeout(now %d)", 3, 10)

    def test_check_sbd_watchdog_timeout_success(self):
        self.instance_check.disk_based = False
        self.instance_check.sbd_watchdog_timeout = 15
        self.instance_check.qdevice_sync_timeout = 10
        self.assertTrue(self.instance_check._check_sbd_watchdog_timeout())

    @patch('crmsh.sbd.SBDManager.update_sbd_configuration')
    def test_fix_sbd_watchdog_timeout(self, mock_update_sbd_configuration):
        self.instance_fix.qdevice_sync_timeout = 10
        self.instance_fix._fix_sbd_watchdog_timeout()
        mock_update_sbd_configuration.assert_called_once_with({"SBD_WATCHDOG_TIMEOUT": str(10+sbd.SBDTimeout.QDEVICE_SYNC_TIMEOUT_MARGIN)})

    @patch('logging.Logger.warning')
    def test_check_sbd_delay_start_failure(self, mock_logger_warning):
        self.instance_check.sbd_delay_start_value_expected = "100"
        self.instance_check.sbd_delay_start_value_from_config = "30"
        self.assertFalse(self.instance_check._check_sbd_delay_start())
        mock_logger_warning.assert_called_once_with("It's recommended that SBD_DELAY_START is set to %s, now is %s", "100", "30")
    ...
    def test_check_sbd_systemd_start_timeout_failure(self, mock_get_sbd_systemd_start_timeout, mock_logger_warning):
        mock_get_sbd_systemd_start_timeout.return_value = 30
        self.instance_check.sbd_systemd_start_timeout_expected = 60
        self.assertFalse(self.instance_check._check_sbd_systemd_start_timeout())
        mock_logger_warning.assert_called_once_with("It's recommended that systemd start timeout for sbd.service is set to %ds, now is %ds", 60, 30)
    ...
    def test_check_stonith_watchdog_timeout_disk_based_failure(self, mock_get_property, mock_logger_warning):
        self.instance_check.disk_based = True
        mock_get_property.return_value = 5
        self.assertFalse(self.instance_check._check_stonith_watchdog_timeout())
        mock_logger_warning.assert_called_once_with("It's recommended that stonith-watchdog-timeout is not set when using disk-based SBD")

    @patch('logging.Logger.warning')
    @patch('crmsh.utils.get_property')
    def test_check_stonith_watchdog_timeout_disk_less_failure(self, mock_get_property, mock_logger_warning):
        self.instance_check.disk_based = False
        self.instance_check.stonith_watchdog_timeout = 15
        mock_get_property.return_value = ""
        self.assertFalse(self.instance_check._check_stonith_watchdog_timeout())
        mock_logger_warning.assert_called_once_with("It's recommended that stonith-watchdog-timeout is set to %d, now is %s", 15, "not set")

    @patch('crmsh.utils.get_property')
    def test_check_stonith_watchdog_timeout_success(self, mock_get_property):
        self.instance_check.disk_based = False
        self.instance_check.stonith_watchdog_timeout = 20
        mock_get_property.return_value = 25
        self.assertTrue(self.instance_check._check_stonith_watchdog_timeout())

    @patch('crmsh.utils.delete_property')
    @patch('logging.Logger.info')
    def test_fix_stonith_watchdog_timeout_disk_based_success(self, mock_logger_info, mock_delete_property):
        self.instance_fix.disk_based = True
        self.instance_fix._fix_stonith_watchdog_timeout()
        mock_logger_info.assert_called_once_with("Removing stonith-watchdog-timeout property")
        mock_delete_property.assert_called_once_with('stonith-watchdog-timeout')

    @patch('crmsh.utils.set_property')
    @patch('logging.Logger.info')
    def test_fix_stonith_watchdog_timeout_disk_less_success(self, mock_logger_info, mock_set_property):
        self.instance_fix.disk_based = False
        self.instance_fix.stonith_watchdog_timeout = 15
        self.instance_fix._fix_stonith_watchdog_timeout()
        mock_logger_info.assert_called_once_with("Adjusting stonith-watchdog-timeout to %d", 15)
        mock_set_property.assert_called_once_with('stonith-watchdog-timeout', 15)

    @patch('logging.Logger.warning')
    @patch('crmsh.utils.get_property')
    def test_check_stonith_timeout_failure(self, mock_get_property, mock_logger_warning):
        self.instance_check.get_stonith_timeout_expected = Mock(return_value=60)
        mock_get_property.return_value = 30
        self.assertFalse(self.instance_check._check_stonith_timeout())
        mock_logger_warning.assert_called_once_with("It's recommended that stonith-timeout is set to %d, now is %s", 60, 30)

    @patch('crmsh.utils.get_property')
    def test_check_stonith_timeout_success(self, mock_get_property):
        self.instance_check.get_stonith_timeout_expected = Mock(return_value=50)
        mock_get_property.return_value = 50
        self.assertTrue(self.instance_check._check_stonith_timeout())

    @patch('crmsh.utils.set_property')
    @patch('logging.Logger.info')
    def test_fix_stonith_timeout(self, mock_logger_info, mock_set_property):
        self.instance_fix.get_stonith_timeout_expected = Mock(return_value=70)
