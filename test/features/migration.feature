...
Feature: migration
  ...
  Scenario: Run pre-migration checks
    ...
    And     Expect stdout contains snippets ["[WARN] Corosync transport \"udpu\" will be deprecated in favor of \"knet\" in corosync 3.", "[FAIL] The pacemaker cluster stack can not migrate to SLES 16. Please fix all the \"FAIL\" problems above before migrating to SLES 16.", "----- corosync @ localhost -----", "----- corosync @ hanode2 -----"].
  ...
  Scenario: Run pre-migration checks with cluster services stopped.
    ...
    Then    Expected return code is "1"
    And     Expect stdout contains snippets ["Check results may be outdated or inaccurate.", "[WARN] Corosync transport \"udpu\" will be deprecated in favor of \"knet\" in corosync 3.", "[FAIL] The pacemaker cluster stack can not migrate to SLES 16. Please fix all the \"FAIL\" problems above before migrating to SLES 16.", "----- corosync @ localhost -----", "----- corosync @ hanode2 -----"].
