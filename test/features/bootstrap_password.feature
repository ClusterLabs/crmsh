...
Feature: crmsh bootstrap process - with password authentication
  ...
  Scenario: Skip creating ssh key pairs when keys are available from ssh-agent fowarding
    ...
    Then This expect program exits with 0 on "root"@"hanode3"
      """
      set timeout 120
      spawn env SSH_AUTH_SOCK=/tmp/ssh-auth-sock ssh -A root@hanode1 crm cluster init -y
      expect "Password: " {
        send "root123\n"
      }
      expect eof
    ...
    Then This expect program exits with 0 on "root"@"hanode3"
      """
      set timeout 120
      spawn env SSH_AUTH_SOCK=/tmp/ssh-auth-sock ssh -A root@hanode2 crm cluster join -c hanode1 -y
      expect "Password: " {
        send "root123\n"
      }
      expect eof
    ...
    Then This expect program exits with 0 on "root"@"hanode3"
      """
      set timeout 120
      spawn env SSH_AUTH_SOCK=/tmp/ssh-auth-sock ssh -A root@hanode1 crm cluster init qdevice --qnetd-hostname qnetd-node -y
      expect "Password: " {
        send "root123\n"
      }
      expect eof
