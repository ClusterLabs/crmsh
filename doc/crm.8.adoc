...
with SBD as the fence device, crmsh configures several fencing related
...
`fencing-watchdog-timeout`: (only for diskless SBD)
...............
    fencing-watchdog-timeout = 2 * SBD_WATCHDOG_TIMEOUT

    # When setting crashdump watchdog timeout
    fencing-watchdog-timeout = max(SBD_WATCHDOG_TIMEOUT + crashdump_watchdog_timeout, 2 * SBD_WATCHDOG_TIMEOUT)
...............
`fencing-timeout`:
...............
    # For disk-based SBD
    _value_from_sbd = 1.2 * msgwait
    fencing-timeout = max(_value_from_sbd, 60s) + corosync.token + corosync.consensus

    # For diskless SBD
    _value_from_sbd = 1.2 * max(fencing-watchdog-timeout, 2*SBD_WATCHDOG_TIMEOUT)
    fencing-timeout = max(_value_from_sbd, 60s) + corosync.token + corosync.consensus

    # For general scenarios
    fencing-timeout = 60s + corosync.token + corosync.consensus
...............
...
...............
health hawk2|sbd|sles16|corosync [--local] [--fix] [--json]
...............
...
* `sles16`: check whether the cluster is good to migrate to SLES 16.
    ...
    ** `--fix`: attempts to automatically resolve any detected issues.
* `corosync`: check corosync-related configurations and status.
    ** `--local`: run checks in local mode.
    ** `--json`: output the check results in JSON format.
...
Note: crmsh's RA tracing relies on the tracing support built into
`ocf-shellfuncs`, and therefore only works for resource agents
implemented as shell scripts. Resource agents written in other
languages, such as most `stonith:fence_*` agents which are written in
Python, will not honor tracing, even though `crm` will still report
that the trace was set. crmsh will emit a warning when it detects that
the specified resource agent is not a shell script.
...
crm sbd configure [watchdog-timeout=<integer>] [allocate-timeout=<integer>] [loop-timeout=<integer>] [msgwait-timeout=<integer>] [crashdump-watchdog-timeout=<integer>] [watchdog-device=<device|driver>] [-F|--force]

# For disk-less SBD
crm sbd configure show [sysconfig|property]
crm sbd configure [watchdog-timeout=<integer>] [crashdump-watchdog-timeout=<integer>] [watchdog-device=<device|driver>] [-F|--force]
...............
...
Multiple nodes may be specified. If no nodes are specified, the
command applies to the local node.

To disable maintenance mode on the node, use `crm node maintenance <node> off`.

Usage:
...............
maintenance [<node>...] [on|off]
...............

[[cmdhelp.node.online,set node online (deprecated),From Code]]
==== `online`
...
[[cmdhelp.node.ready,put node into ready mode (deprecated)]]
==== `ready`
...
Note: The `ready` command is deprecated and will be removed in a future release.
Use `crm node maintenance <node> off` instead.
...
Example:
...............
property fencing-enabled --help
property fencing-enabled=true
property rule date spec years=2014 fencing-enabled=false
...............
