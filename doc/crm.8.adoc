...
For any detected `[WARN]` issues, the optional `--fix` argument attempts to
resolve them only when all nodes are ssh reachable and the major versions of
pacemaker and corosync on all nodes are identical.

For all detected `[FAIL]` issues, they must be manually addressed beforehand.
`--fix` won't be able to resolve it.
****************************
