...
...............
    ...
    # When setting crashdump watchdog timeout
    stonith-watchdog-timeout = max(SBD_WATCHDOG_TIMEOUT + crashdump_watchdog_timeout, 2 * SBD_WATCHDOG_TIMEOUT)
...............
`stonith-timeout`:
...............
    # For disk-based SBD
    value_from_sbd = 1.2 * msgwait
    stonith-timeout = max(value_from_sbd, 60s) + corosync.token + corosync.consensus
