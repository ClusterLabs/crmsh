...
#					envir.USER_CLUSTER_TYPE = 'openais'
	...
	def start_slave_collector(self,nodes,port=22,username='root'):
		...
		command= 'python '+envir.EXCUTE_PATH+'/hb_report --slave'
		stdin,stdout,stderr = client.exec_command(command)
		
		out_msg = nodes+ ' output :'+stdout.read()
		utillib.writefile(os.path.join(self.WORKDIR,'output.txt'),out_msg)
		err_msg = nodes+ ' error: '+stderr.read()
		utillib.writefile(os.path.join(self.WORKDIR,'errput.txt'),err_msg)
