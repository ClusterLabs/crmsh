...
def check():
    has_problems = [False]
    def problem_handler(is_fatal: bool, title: str, detail: str):
        has_problems[0] = True
        logger.error('%s\n%s', title, detail)
        if is_fatal:
...
def check_dependency_version(handler: typing.Callable[[bool, str, str], None]):
    ...
    if not match or match.group(1) != '3':
        handler(False, 'Corosync version not supported', f'Supported version: corosync >= 3\n{out}')


def check_service_status(handler: typing.Callable[[bool, str, str], None]):
    ...
    if active_services:
        handler(False, 'Cluster services are running', '\n'.join(f'* {x}' for x in active_services))
