...
def check_remote():
    handler = CheckResultInteractiveHandler()
    results = parallax.parallax_call(
        utils.list_cluster_nodes_except_me(),
        'crm cluster health sles16 --local --no-check-cib --json=oneline',
        strict=False,
    )
    ret = 0
    for host, result in results:
        ...
        if isinstance(result, parallax.Error):
                ...
                if rc > 1:
                    print(stdout.decode('utf-8', 'backslashreplace'))
                    handler.write_in_color(
                        sys.stdout, constants.YELLOW,
                    ...
                    try:
                        ...
                        passed = result_json.get("pass", False)
                        handler = CheckResultInteractiveHandler()
                        for problem in result_json.get("problems", list()):
                            handler.handle_problem(False, problem.get("title", ""), problem.get("descriptions"))
                        for tip in result_json.get("tips", list()):
...
def check_dependency_version(handler: CheckResultHandler):
    ...
    out = utils.get_stdout_or_raise_error('corosync -v', None)
    _check_version_range(
        handler,
        'Corosync', (2, 4, 6), (3,),
        re.compile(r"version\s+'(\d+(?:\.\d+)*)'"),
        utils.get_stdout_or_raise_error('corosync -v', None),
    )
    _check_version_range(
        handler,
        'Pacemaker', (2, 1, 7), (3,),
...
def check_service_status(handler: CheckResultHandler):
    ...
    inactive_services = [x for x in ['corosync', 'pacemaker'] if not utils.service_is_active(x)]
...
def check_unsupported_resource_agents(handler: CheckResultHandler):
    ...
    cib = xmlutil.text2elem(utils.get_stdout_or_raise_error('crm configure show xml', None))
