...
class NodeMgmt(command.UI):
    ...
    def _standby_on(self, node_list, lifetime_opt):
        ...
        if not rc:
            return False
        for node in node_list:
            logger.info("Setting standby=true on node %s", node)
    ...
    def _standby_off(self, node_list):
        ...
        if not rc:
            return False
        for node in node_list:
            logger.info("Setting standby=false on node %s", node)
    ...
    def do_fence(self, context, node):
        ...
        if not utils.has_fence_device_registered():
            logger.error("fence command requires fence device configured and running")
            return False
