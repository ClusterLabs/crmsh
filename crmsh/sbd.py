...
class SBDTimeout(object):
    ...
    def get_sbd_systemd_start_timeout_expected(self):
...
class SBDTimeoutChecker(SBDTimeout):
    ...
    def _check_config_consistency(self) -> bool:
        ...
        if diff_output:
            logger.warning("corosync.conf is not consistent across cluster nodes")
            consistent = False

        diff_output = utils.remote_diff_this(SBDManager.SYSCONFIG_SBD, other_nodes, this_node)
        if diff_output:
            logger.warning("%s is not consistent across cluster nodes", SBDManager.SYSCONFIG_SBD)
            consistent = False
        ...
        if not SBDUtils.check_devices_metadata_consistent(configured_devices):
            logger.warning("SBD device metadata is not consistent across cluster nodes")
            consistent = False

        if not consistent:
            logger.warning("Please ensure the configurations are consistent across all cluster nodes")
        return consistent
