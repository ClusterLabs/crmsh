...
FILES_TO_SYNC = (BOOTH_DIR, corosync.conf(), COROSYNC_AUTH, CSYNC2_CFG, CSYNC2_KEY, "/etc/ctdb/nodes",
        ...
        "/etc/samba/smb.conf", SYSCONFIG_PCMK, SYSCONFIG_SBD, WATCHDOG_CFG)
...
def configure_firewall(tcp=None, udp=None):
    ...
    def init_firewall_firewalld(tcp, udp):
        for p in tcp:
            if not invokerc("firewall-cmd --zone=public --add-port={}/tcp --permanent".format(p)):
                error("Failed to configure firewall (firewalld via firewall-cmd)")

        for p in udp:
            if not invokerc("firewall-cmd --zone=public --add-port={}/udp --permanent".format(p)):
                error("Failed to configure firewall (firewalld via firewall-cmd)")
...
def configure_local_ssh_key(user="root"):
    ...
    key_dict = key_files(user)
    private_key = key_dict['private']
    public_key = key_dict['public']
    authorized_file = key_dict['authorized']
...
def create_booth_authkey():
    ...
    if not invokerc("booth-keygen {}".format(BOOTH_AUTH)):
