...
INIT_STAGES = ("ssh", "csync2", "csync2_remote", "corosync", "storage", "sbd", "cluster", "vgfs", "admin")
...
class Context(object):
    ...
    def validate_option(self):
        ...
        if self.admin_ip:
            try:
                Validation.valid_adminIP(self.admin_ip)
...
class Validation(object):
    ...
    @classmethod
    def valid_ucastIP(cls, addr, prev_value_list=[]):
        """
    ...
    @classmethod
    def valid_mcastIP(cls, addr, prev_value_list=[]):
        """
    ...
    @staticmethod
    def valid_adminIP(addr, prev_value_list=[]):
        """
        ...
        if not _context.interfaces_inst.ip_in_network(addr):
...
""".format(_context.default_nic_list[0]))
    ...
    for i in range(2):
        ringXaddr = prompt_for_string(
                ...
                valid_func=Validation.valid_ucastIP,
...
""".format(_context.default_nic_list[0]))
    ...
    for i in range(2):
        bindnetaddr = prompt_for_string(
                ...
                valid_func=Validation.valid_mcastIP,
...
""")
        ...
        adminaddr = prompt_for_string('Virtual IP', valid_func=Validation.valid_adminIP)
        if not adminaddr:
