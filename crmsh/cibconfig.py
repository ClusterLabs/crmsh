...
def show_unrecognized_elems(cib_elem):
    ...
    for topnode in conf.iterchildren():
        if xmlutil.is_defaults(topnode) or topnode.tag == "fencing-topology":
            continue
...
def copy_nvpair(nvpairs, nvp, id_hint=None):
    ...
    """
    logger.debug("copy_nvpair: %s", xmlutil.xml_tostring(nvp))
...
def copy_nvpairs(tonode, fromnode):
    ...
    def copy_comment(cnode):
        for nvp2 in tonode:
            if xmlutil.is_comment(nvp2) and nvp2.text == cnode.text:
    ...
    logger.debug("copy_nvpairs: %s -> %s", xmlutil.xml_tostring(fromnode), xmlutil.xml_tostring(tonode))
    id_hint = tonode.get('id')
    for c in fromnode:
        if xmlutil.is_comment(c):
            copy_comment(c)
        elif c.tag == "nvpair":
...
class CibObjectSet(object):
    ...
    def _graph_repr(self, gv_obj):
        ...
        '''
        for obj in xmlutil.processing_sort_cli(list(self.obj_set)):
    ...
    def _check_unique_clash(self, set_obj_all):
        ...
        def process_primitive(prim, clash_dict):
            ...
            ra_inst = ra.get_ra(r_node)
            if ra_inst.mk_ra_node() is None:  # no RA found?
                return
            ra_params = ra_inst.params()
    ...
    def semantic_check(self, set_obj_all) -> utils.VerifyResult:
        '''
...
class CibObjectSetCli(CibObjectSet):
    ...
    def save(self, s, remove=True, method='replace'):
        ...
        with logger_utils.line_number():
            for cli_text in utils.lines2cli(s):
...
class CibObjectSetRaw(CibObjectSet):
    ...
    def repr(self, format_mode="ignored"):
        ...
        for nvp in cib_elem.xpath('//nvpair'):
            if 'value' in nvp.attrib:
                nvp.set('value', utils.obscured(nvp.get('name'), nvp.get('value')))

        s = xmlutil.xml_tostring(cib_elem, pretty_print=True)
    ...
    def save(self, s, remove=True, method='replace'):
        try:
            ...
            return False
        xmlutil.sanitize_cib(cib_elem)
        if not show_unrecognized_elems(cib_elem):
            return False
        rc = True
        diff = CibDiff(self)
        for node in xmlutil.get_top_cib_nodes(cib_elem, []):
            rc = diff.add(node)
        if not rc:
            return rc
    ...
    def verify(self) -> utils.VerifyResult:
        if not self.obj_set:
            return utils.VerifyResult.SUCCESS
        with clidisplay.nopretty():
        ...
        if rc not in (0, 1):
            ...
            return utils.VerifyResult.NON_FATAL_ERROR
        return utils.VerifyResult.SUCCESS
    ...
    def ptest(self, nograph, scores, utilization, actions, verbosity):
        ...
        return utils.run_ptest(graph_s, nograph, scores, utilization, actions, verbosity)


def find_comment_nodes(node):
    return [c for c in node.iterchildren() if xmlutil.is_comment(c)]
...
def resolve_idref(node):
    ...
    if obj:
        ...
        if len(nodes) > 0:
            ...
            if node_id:
                return node_id
    xmlutil.check_id_ref(cib_factory.get_cib(), id_ref)
    return id_ref
        ...
        if node.tag == 'op':
            ...
            if defid is not None:
                try:
                    ...
                    raise ValueError('Internal error: %s (%s)' % (e, xmlutil.xml_tostring(node)))
                obj_id = node.get('id')
                idmgmt.save(obj_id)
    if root.tag != "node" and obj_id and not utils.is_id_valid(obj_id):
...
def postprocess_cli(node, oldnode=None, id_hint=None, promotable_default_meta=False):
    ...
    if obj_id is None:
        ...
        logger.error("No ID found for %s: %s", obj_type, xmlutil.xml_tostring(node))
    ...
    if oldnode is not None:
        xmlutil.remove_id_used_attributes(oldnode)
...
class CibObject(object):
    ...
    def _repr_cli_xml(self, format_mode):
        with clidisplay.nopretty(format_mode < 0):
            ...
            l = xmlutil.xml_tostring(self.node, pretty_print=True).split('\n')
            l = [x for x in l if x]  # drop empty lines
            return "%s %s" % (h, cliformat.cli_format(l, break_lines=(format_mode > 0), xml=True))
    ...
    def repr_cli(self, format_mode=1):
        ...
        with clidisplay.nopretty(format_mode < 0):
            ...
            if desc:
                l.append(cliformat.nvpair_format("description", desc))
    ...
    def _attr_set_str(self, node):
        ...
        if node_id is not None and cib_factory.is_id_refd(node.tag, node_id):
            ret += "%s " % (cliformat.nvpair_format("$id", node_id))
        elif idref is not None:
            ret += "%s " % (cliformat.nvpair_format("$id-ref", idref))
        ...
        for c in node.iterchildren():
            if c.tag == "rule":
                ret += "%s %s " % (clidisplay.keyword("rule"), cliformat.cli_rule(c))
        for c in node.iterchildren():
            if c.tag == "nvpair":
                ret += "%s " % (cliformat.cli_nvpair(c))
        if ret[-1] == ' ':
            ret = ret[:-1]
        return ret
    ...
    def _attr_set_bundle_str(self, node):
        ...
        if node.tag in constants.container_type:
            if "image" not in node.keys():
                ...
                return ret
            ret += f"{cliformat.nvpair_format('image', node.get('image'))} "
            for item in node.keys():
                if item != "image":
                    ret += f"{prefix}{cliformat.nvpair_format(item, node.get(item))} "

        if node.tag in ("network", "storage"):
            for item in node.keys():
                ret += f"{cliformat.nvpair_format(item, node.get(item))} "
            for _type in ("port-mapping", "storage-mapping"):
                for c in node.iterchildren(_type):
                    ret += f"{prefix}{_type} "
                    for item in c.keys():
                        if item != "id":
                            ret += f"{cliformat.nvpair_format(item, c.get(item))} "
    ...
    def _get_oldnode(self):
        ...
        if self.obj_type == "property":
            return xmlutil.get_topnode(cib_factory.get_cib(), self.parent_type)
    ...
    def _cli_format_and_comment(self, l, comments, format_mode):
        ...
        '''
        s = cliformat.cli_format(l, break_lines=(format_mode > 0))
        cs = '\n'.join(comments)
    ...
    def cli_use_validate(self):
        ...
        if not cli_text:
            logger.debug("validation failed: %s", xmlutil.xml_tostring(self.node))
            return False
        xml2 = self.cli2node(cli_text)
        if xml2 is None:
            logger.debug("validation failed: %s -> %s", xmlutil.xml_tostring(self.node), cli_text)
            return False
        if not xmlutil.xml_equals(self.node, xml2, show=True):
            logger.debug("validation failed: %s -> %s -> %s", xmlutil.xml_tostring(self.node), cli_text, xmlutil.xml_tostring(xml2))
            return False
        return True
    ...
    def check_sanity(self) -> utils.VerifyResult:
        '''
...
def gv_edge_score_label(gv_obj, e_id, node):
    score = cliformat.get_score(node) or cliformat.get_kind(node)
    if cliformat.abs_pos_score(score):
...
def reduce_primitive(node):
    ...
    if not template_obj:
        return None
    return xmlutil.merge_tmpl_into_prim(node, template_obj.node)
...
class Op(object):
    ...
    def mkxml(self):
        ...
        inst_attr = {}
        valid_attrs = utils.olist(schema.get('attr', 'op', 'a'))
...
class CibOp(CibObject):
    ...
    def _repr_cli_head(self, format_mode):
        action, pl = xmlutil.op2list(self.node)
        if not action:
            return ""
        ret = ["%s %s" % (clidisplay.keyword("op"), action)]
        ret += [cliformat.nvpair_format(n, v) for n, v in pl]
        return ' '.join(ret)
...
class CibPrimitive(CibObject):
    ...
    def _repr_cli_head(self, format_mode):
        ...
        if template_ref:
            ...
            rsc_spec = xmlutil.mk_rsc_type(self.node)
        s = clidisplay.keyword(self.obj_type)
    ...
    def _repr_cli_child(self, c, format_mode):
        if c.tag in self.set_names:
            ...
            c_id = c.get("id")
            if c_id:
                s = cliformat.nvpair_format('$id', c_id)
            idref = c.get("id-ref")
            if idref:
                s = '%s %s' % (s, cliformat.nvpair_format('$id-ref', idref))
            if s:
            ...
            return cliformat.cli_format(l, break_lines=(format_mode > 0))
    ...
    def add_operation(self, node):
        ...
        if xmlutil.find_operation(self.node, name, interval) is not None:
        ...
        if comments and self.node is not None:
            xmlutil.stuff_comments(self.node, [c.text for c in comments])
        self.set_updated()
        return self
    ...
    def is_dummy_operation(self, op_node):
        ...
        if len(op_node) == 0 and utils.crm_msec(interval) == 0:
    ...
    def normalize_parameters(self):
        ...
        if r_node is None:
            return
        ra_inst = ra.get_ra(r_node)
        ra_inst.normalize_parameters(r_node)
    ...
    def check_sanity(self) -> utils.VerifyResult:
        ...
        rc3 = xmlutil.sanity_check_meta(self.obj_id, self.node, ra.get_resource_meta_list())
        if self.obj_type == "primitive":
            r_node = reduce_primitive(self.node)
            if r_node is None:
        ...
        if ra_inst.mk_ra_node() is None:  # no RA found?
            ...
            if config.core.ignore_missing_metadata:
                return rc3
            ra_inst.error("no such resource agent")
            return utils.get_check_rc()
        actions = xmlutil.get_rsc_operations(r_node)
        default_timeout = get_default_timeout()
        rc2 = ra_inst.sanity_check_ops(self.obj_id, actions, default_timeout)
        rc4 = self._check_ops_attributes()
        params = []
        for c in r_node.iterchildren("instance_attributes"):
            params += cliformat.nvpairs2list(c)
        rc1 = ra_inst.sanity_check_params(self.obj_id,
                                     params,
...
class CibContainer(CibObject):
    ...
    def _repr_cli_head(self, format_mode):
        children = []
        for c in self.node.iterchildren():
            if (self.obj_type == "group" and xmlutil.is_primitive(c)) or \
                    xmlutil.is_child_rsc(c):
                children.append(clidisplay.rscref(c.get("id")))
            elif self.obj_type in constants.clonems_tags and xmlutil.is_child_rsc(c):
                children.append(clidisplay.rscref(c.get("id")))
        s = clidisplay.keyword(self.obj_type)
        ident = clidisplay.ident(self.obj_id)
    ...
    def check_sanity(self) -> utils.VerifyResult:
        ...
        rc |= xmlutil.sanity_check_meta(self.obj_id, self.node, l)
        return rc
...
def _check_if_constraint_ref_is_child(obj) -> utils.VerifyResult:
    """
...
def _check_if_primitive_in_constraint_is_remote(obj) -> utils.VerifyResult:
    rc = utils.VerifyResult.SUCCESS
    primitives = []
    if obj.obj_type == "colocation":
...
class CibLocation(CibObject):
    ...
    def _repr_cli_head(self, format_mode):
        ...
        if rsc is not None:
            ...
            rsc = '{ %s }' % (' '.join(cliformat.rsc_set_constraint(self.node, self.obj_type)))
        else:
        ...
        score = clidisplay.score(cliformat.get_score(self.node))
    ...
    def _repr_cli_child(self, c, format_mode):
        if c.tag == "rule":
            return "%s %s" % \
                (clidisplay.keyword("rule"), cliformat.cli_rule(c))
    ...
    def repr_gv(self, gv_obj, from_grp=False):
        ...
        elif xmlutil.is_pref_location(self.node):
...
class CibSimpleConstraint(CibObject):
    ...
    def _repr_cli_head(self, format_mode):
        s = clidisplay.keyword(self.obj_type)
        ident = clidisplay.ident(self.obj_id)
        score = cliformat.get_score(self.node) or cliformat.get_kind(self.node)
        if self.node.find("resource_set") is not None:
            col = cliformat.rsc_set_constraint(self.node, self.obj_type)
        else:
            col = cliformat.simple_rsc_constraint(self.node, self.obj_type)
        if not col:
            return None
    ...
    def _mk_optional_set(self, gv_obj, n):
        ...
        '''
        members = xmlutil.get_rsc_ref_ids(n)
        sg_name = _opt_set_name(n)
    ...
    def repr_gv(self, gv_obj, from_grp=False):
        ...
        if self.node.find("resource_set") is not None:
            ...
            for n in self.node.iterchildren("resource_set"):
                if not utils.get_boolean(n.get("require-all"), True):
...
class CibProperty(CibObject):
    ...
    def _repr_cli_head(self, format_mode):
        return "%s %s" % (clidisplay.keyword(self.obj_type),
                          cliformat.head_id_format(self.obj_id))

    def _repr_cli_child(self, c, format_mode):
        if c.tag == "rule":
            return ' '.join((clidisplay.keyword("rule"),
                             cliformat.cli_rule(c)))
        elif c.tag == "nvpair":
            return cliformat.cli_nvpair(c)
        else:
            return ''
    ...
    def _repr_cli_head(self, format_mode):
        ...
        def fmt_target(tgt):
            if isinstance(tgt, tuple):
                ...
                return "attr:%s=%s" % tgt
            return tgt + ":"
        return cliformat.cli_format([s] + ["%s %s" % (fmt_target(x), ' '.join(dd[x]))
...
class CibAcl(CibObject):
    ...
    def _repr_cli_child(self, c, format_mode):
        if c.tag in constants.acl_rule_names:
            return cliformat.cli_acl_rule(c, format_mode)
        elif c.tag == "role_ref":
            return cliformat.cli_acl_roleref(c, format_mode)
        elif c.tag == "role":
            return cliformat.cli_acl_role(c)
        elif c.tag == "acl_permission":
            return cliformat.cli_acl_permission(c)
    ...
    def _repr_cli_head(self, fmt):
        return ' '.join([clidisplay.keyword(self.obj_type),
                        ...
                        [clidisplay.rscref(c.get('id'))
                         for c in self.node.iterchildren() if not xmlutil.is_comment(c)])
...
class CibAlert(CibObject):
    ...
    def _repr_cli_head(self, fmt):
        ret = [clidisplay.keyword(self.obj_type),
               clidisplay.ident(self.obj_id),
               cliformat.cli_path(self.node.get('path'))]
        return ' '.join(ret)
    ...
    def _repr_cli_child(self, c, format_mode):
        if c.tag in self.set_names:
            ...
            if is_complex:
                r.append('{')
            r.append(cliformat.cli_path(c.get('value')))
...
class CibDiff(object):
    ...
    def add(self, item):
        ...
        if obj_id is None:
            logger.error("element %s has no id!", xmlutil.xml_tostring(item, pretty_print=True))
            return False
...
class CibFactory(object):
    ...
    def change_schema(self, schema_st):
        ...
        for obj in self.cib_objects:
            if schema.get('sub', obj.node.tag, 'a') is None:
                ...
                logger.debug("Offending object: %s", xmlutil.xml_tostring(obj.node))
                rc = False
        if not rc:
    ...
    def upgrade_validate_with(self, force=False):
        ...
        if force or not validator:
            return utils.ext_cmd("cibadmin --upgrade --force") == 0
    ...
    def obj_set2cib(self, obj_set, obj_filter=None):
        ...
        for obj in obj_set:
            ...
            d[obj.top_parent()] = 1
        for obj in d:
            xmlutil.get_topnode(cib_elem, obj.parent_type).append(copy.deepcopy(obj.node))
    ...
    def is_current_cib_equal(self, silent=False):
        cib_elem = xmlutil.read_cib(xmlutil.cibdump2elem)
        if cib_elem is None:
            return False
    ...
    def _update_schema(self):
        ...
        rc = utils.pipe_string("%s -U" % cib_piped, s)
        if rc != 0:
    ...
    def _replace_cib(self, force):
        ...
        rc = utils.pipe_string("%s %s" % (cib_piped, cibadmin_opts), etree.tostring(conf_el))
        if rc != 0:
            logger_utils.update_err("cib", cibadmin_opts, xmlutil.xml_tostring(conf_el), rc)
            return False
        return True
    ...
    def _patch_cib(self, force):
        ...
        # to apply the patch)
        current_cib = xmlutil.read_cib(xmlutil.cibdump2elem)
        ...
        cib_s = xmlutil.xml_tostring(self.cib_orig, pretty_print=True)
        tmpf = utils.str2tmp(cib_s, suffix=".xml")
        if not tmpf or not utils.ensure_sudo_readable(tmpf):
            return False
        tmpfiles.add(tmpf)
        ...
        logger.debug("Input: %s", xmlutil.xml_tostring(self.cib_elem))
        rc, cib_diff = utils.filter_string("%s -o %s -n -" %
        ...
        rc = utils.pipe_string("%s %s" % (cib_piped, cibadmin_opts),
    ...
    def _populate(self):
        ...
        all_nodes = xmlutil.get_interesting_nodes(self.cib_elem, [])
        if not all_nodes:
            return
        for node in xmlutil.processing_sort(all_nodes):
            if xmlutil.is_defaults(node):
    ...
    def initialize(self, cib=None, no_side_effects=False):
        if self.cib_elem is not None:
            return True
        if cib is None:
            cib_element = xmlutil.read_cib(lambda x: xmlutil.cibdump2elem(x, no_side_effects=no_side_effects))
            if cib_element is None and no_side_effects:
                return False
        elif isinstance(cib, str):
            cib_element = xmlutil.text2elem(cib)
        else:
            cib_element = cib
        if not self._import_cib(cib_element):
            return False
        self.cib_orig = copy.deepcopy(self.cib_elem)
        xmlutil.sanitize_cib_for_patching(self.cib_orig)
        xmlutil.sanitize_cib(self.cib_elem)
        ...
        for obj_id in args:
            ...
            if ra_inst.mk_ra_node() is None:  # no RA found?
                if not self.is_asymm_cluster():
                    ra_inst.error("no resource agent found for %s" % obj_id)
                continue
            obj_modified = False
            for c in r_node.iterchildren():
                if c.tag == "operations":
                    for c2 in c.iterchildren():
                        ...
                        elif xmlutil.is_ms_or_promotable_clone(obj.node.getparent()) and op in implied_ms_actions:
                        ...
                        role = c2.get('role')
                        depth = c2.get('depth')
                        adv_timeout = ra_inst.get_op_attr_value(op, "timeout", role=role, depth=depth)
                        if adv_timeout:
            ...
            if xmlutil.is_ms_or_promotable_clone(obj.node.getparent()):
                l += implied_ms_actions
            for op in l:
                adv_timeout = ra_inst.get_op_attr_value(op, "timeout")
                if not adv_timeout:
                    continue
                n = etree.Element('op')
                n.set('name', op)
    ...
    def resolve_id_ref(self, attr_list_type, id_ref):
        ...
        if obj:
            ...
            if numnodes > 0:
                ...
                if node_id:
                    return node_id
        xmlutil.check_id_ref(self.cib_elem, id_ref)
        return id_ref
    ...
    def _get_attr_value(self, obj_type, attr):
        ...
        for obj in self.cib_objects:
            if obj.obj_type == obj_type and obj.node is not None:
                for n in cliformat.nvpairs2list(obj.node):
                    if n.get('name') == attr:
                        return n.get('value')
        return None
    ...
    def get_property_w_default(self, prop):
        ...
        if v is None:
            try:
                v = ra.get_properties_meta().param_default(prop)
            except:
                pass
        return v
    ...
    def _verify_constraints(self, node):
        ...
        for obj_id in xmlutil.referenced_resources(node):
    ...
    def _verify_rsc_children(self, obj):
        ...
        for c in obj.node.iterchildren():
            if not xmlutil.is_cib_element(c):
                continue
            child_id = c.get("id")
        ...
        for other in [x for x in self.cib_objects
                      if x != obj and xmlutil.is_container(x.node)]:
    ...
    def _verify_element(self, obj):
        ...
        try:
            ...
            return False
        if xmlutil.is_container(node):
            rc &= self._verify_rsc_children(obj)
        elif xmlutil.is_constraint(node):
    ...
    def set_property_cli(self, obj_type, node):
        ...
        if not obj and pset_id == default_id_for_obj(obj_type):
            ...
            if objs and len(objs) > 0:
                obj = objs[-1]
        if not obj:
            if not utils.is_id_valid(pset_id):
                logger_utils.invalid_id_err(pset_id)
                return None
            obj = self.new_object(obj_type, pset_id)
            if not obj:
                return None
            topnode = xmlutil.get_topnode(self.cib_elem, obj.parent_type)
    ...
    def create_from_cli(self, cli):
        ...
        if elem is None:
            ...
            return None
        logger.debug("create_from_cli: %s, %s, %s", xmlutil.xml_tostring(elem), obj_type, obj_id)
        if obj_type in utils.olist(constants.nvset_cli_names):
    ...
    def update_element(self, obj, newnode):
        ...
        if not self.is_cib_sane():
            ...
            return False
        oldnode = obj.node
        if xmlutil.xml_equals(oldnode, newnode):
        ...
        if not self._adjust_children(obj):
            return False
        if not obj.cli_use_validate():
            logger.debug("update_element: validation failed (%s, %s)", obj, xmlutil.xml_tostring(newnode))
            obj.nocli_warn = True
            obj.nocli = True
        obj.set_updated()
        return True
    ...
    def merge_from_cli(self, obj, node):
        logger.debug("merge_from_cli: %s %s", obj.obj_type, xmlutil.xml_tostring(node))
        if obj.obj_type in constants.nvset_cli_names:
            rc = xmlutil.merge_attributes(obj.node, node, "nvpair")
        else:
            rc = xmlutil.merge_nodes(obj.node, node)
        if rc:
            obj.set_updated()
        return True
    ...
    def _cli_set_update(self, edit_d, mk_set, upd_set, del_set, method):
        ...
        def obj_is_container(x):
            ...
            return obj and xmlutil.is_container(obj.node)

        def obj_is_constraint(x):
            obj = self.find_resource(x)
            return obj and xmlutil.is_constraint(obj.node)
        ...
        for cli in xmlutil.processing_sort([edit_d[x] for x in mk_set]):
            obj = self.create_from_cli(cli)
            if not obj:
                logger.debug("create_from_cli '%s' failed", xmlutil.xml_tostring(cli, pretty_print=True))
                return False
            test_l.append(obj)
        ...
        for ident in upd_set:
            ...
            if not self.update_from_cli(obj, node, method):
                logger.debug("update_from_cli failed: %s, %s, %s", obj, xmlutil.xml_tostring(node), method)
                return False
            test_l.append(obj)
    ...
    def _xml_set_update(self, edit_d, mk_set, upd_set, del_set):
        ...
        test_l = []
        for el in xmlutil.processing_sort([edit_d[x] for x in mk_set]):
    ...
    def _adjust_children(self, obj):
        ...
        '''
        new_children_ids = xmlutil.get_rsc_children_ids(obj.node)
    ...
    def _relink_child_to_top(self, obj):
        ...
        xmlutil.get_topnode(self.cib_elem, obj.parent_type).append(obj.node)
        obj.parent = None
    ...
    def _add_element(self, obj, node):
        ...
        obj.node = node
        obj.set_id()
        pnode = xmlutil.get_topnode(self.cib_elem, obj.parent_type)
    ...
    def create_from_node(self, node):
        ...
        try:
            ...
            return None
        if xmlutil.is_defaults(node):
            node = xmlutil.get_rscop_defaults_meta_node(node)
            if node is None:
                logger.debug("create_from_node: xmlutil.get_rscop_defaults_meta_node failed")
                return None
    ...
    def _remove_obj(self, obj):
        ...
        for c_obj in self.related_constraints(obj):
            if xmlutil.is_simpleconstraint(c_obj.node) and obj.children:
                # the first child inherits constraints
                xmlutil.rename_rscref(c_obj, obj.obj_id, obj.children[0].obj_id)
            deleted = False
            if xmlutil.delete_rscref(c_obj, obj.obj_id):
                deleted = True
            if xmlutil.silly_constraint(c_obj.node, obj.obj_id):
    ...
    def related_constraints(self, obj):
        def related_constraint(obj2):
            return xmlutil.is_constraint(obj2.node) and xmlutil.rsc_constraint(obj.obj_id, obj2.node)
    ...
    def related_elements(self, obj):
        ...
        if not xmlutil.is_resource(obj.node):
            return []
        return [x for x in self.cib_objects if xmlutil.is_related(obj.obj_id, x.node)]
    ...
    def _redirect_children_constraints(self, obj):
        ...
        for child in obj.children:
            for c_obj in self.related_constraints(child):
                xmlutil.rename_rscref(c_obj, child.obj_id, obj.obj_id)
    ...
    def template_primitives(self, obj):
        if not xmlutil.is_template(obj.node):
            return []
        c_list = []
        for obj2 in self.cib_objects:
            if not xmlutil.is_primitive(obj2.node):
                continue
    ...
    def delete(self, *args):
        ...
        for obj_id in args:
            ...
            if not rscstat.can_delete(obj_id):
                ...
                rc = False
                continue
            if xmlutil.is_template(obj.node):
            ...
            l.append(obj)
        if l:
            l = xmlutil.processing_sort_cli(l)
    ...
    def rename(self, old_id, new_id):
        ...
        if not obj.can_be_renamed():
            return False
        for c_obj in self.related_constraints(obj):
            xmlutil.rename_rscref(c_obj, old_id, new_id)
        xmlutil.rename_id(obj.node, old_id, new_id)
        obj.obj_id = new_id
    ...
    def erase(self):
        ...
        l = []
        rscstat = xmlutil.RscState()
        for obj in [obj for obj in self.cib_objects if not obj.children and not xmlutil.is_constraint(obj.node) and obj.obj_type != "node"]:
