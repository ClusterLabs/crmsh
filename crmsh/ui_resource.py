...
_attrcmds = compl.choice(constants.ATTR_OPERATION_CMDS)
...
class RscMgmt(command.UI):
    ...
    def do_failcount(self, context, rsc, cmd, node, value=None, operation=None, interval=None):
        ...
        def sec_to_ms(s):
            return '0' if s == '0' else f'{s}000'

        def ms_to_sec(m):
            return '0' if m == '0' else f'{int(m) // 1000}'
        ...
        if rsc not in compl.resources():
            ...
            return
        if cmd not in constants.ATTR_OPERATION_CMDS:
            context.fatal_error(f"{cmd} is not valid command(should be one of {', '.join(constants.ATTR_OPERATION_CMDS)})")
