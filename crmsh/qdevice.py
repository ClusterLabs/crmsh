...
class QDevice(object):
    ...
    def check_corosync_qdevice_available(self):
        if self.is_stage:
            nodes = utils.list_cluster_nodes() or [utils.this_node()]
        else:
    ...
    def init_db_on_cluster(self, log: typing.Callable[[str, typing.Optional[str]], None]):
        ...
        """
        node_list = utils.list_cluster_nodes() or [utils.this_node()]
    ...
    def remove_qdevice_db(addr_list=[]):
        ...
        if not os.path.exists(QDevice.qdevice_db_path):
            return
        node_list = addr_list or utils.list_cluster_nodes() or [utils.this_node()]
